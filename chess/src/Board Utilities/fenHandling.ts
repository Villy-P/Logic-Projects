...
class FenHandling {
	public static readonly startWhiteFenString: string =
		...
		for (const char of FENSTRING[0].split("")) {
			if (char === "/") {
				...
				if (!Number.isNaN(Number(char))) {
					...
					Board.pieces[rank][file] = new Piece(
						Piece.pieceIdentifiers.get(char)!,
						file,
						rank
					);
					file++;
		...
		for (const array of board) {
			...
			for (const piece of array) {
				if (piece.value === 0) {
					...
					if (piece.value != 0) fen += piece.value.toString();
				}
			}
